...
test("proposal compilation restores a unique equal-length full-title transformation", async () => {
...
test("matching synopsis and attribution can prove a merge when stored titles differ", async () => {
	...
	try {
		...
		const withAuthor = (book: BookEvidence): BookEvidence => {
		...
		const source = withAuthor(sourceBook(randomUUID(), "原名"));
		const target = withAuthor(sourceBook(randomUUID(), "不同标题"));
